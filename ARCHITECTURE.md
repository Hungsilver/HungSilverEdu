...
| `/api/exams` · `/generate/{materialId}` | GET · POST | Teacher/Admin | Danh sách đề (theo môn/tài liệu, paged) · sinh đề bằng AI |
...
- **Service/API:** `ExamGenerationService` (Infrastructure, orchestrate §A) + `ExamService` (Application: paged theo môn/tài liệu,
  detail kèm `SourceFileUrl`, sửa đề/câu qua `ExamQuestionFactory` dùng chung, publish, soft-delete cascade con) +
  `MaterialService.GetPagedBySubjectAsync`. `ExamsController` (`/api/exams`, `[Authorize(TeacherOrAdmin)]`) + `MaterialsController` `by-subject`.
- **FE:** `core/exam.service.ts`; Kho tài liệu thêm mode **"Theo môn"** (lưới phân trang + nút **Đề**); `features/exams/exam-list.page.ts`
  (bảng đề + modal "Tạo đề bằng AI" Extract/Generate, loading ~1') → `exam-detail.page.ts` (**PDF gốc song song** tải qua HttpClient
...
- **Triển khai:** image API cài **LibreOffice** (`server/Dockerfile`); config `DocumentConversion` (SofficePath trống=PATH). Sinh đề
  **đồng bộ** (~30–90s); GV cần cấu hình API Key Gemini ở `/profile` trước (lỗi `Ai.KeyMissing` nếu thiếu).
