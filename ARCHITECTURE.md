...
- **Quan hệ hiện hành phải cleanup khi parent soft-delete:** không để bảng nối/trạng thái "đang hiệu lực" tiếp tục active nếu parent đã bị xóa mềm. `ICurrentRelationCleanupService` xử lý các ca chung: soft-delete `Enrollment` active + set `IsActive=false`/`WithdrawnOn`, dọn enrollment active mồ côi trước khi xóa lớp, unlink `Student.UserId`/`TeacherProfile.UserId` khi xóa user, null `Assignment.MaterialId` khi xóa học liệu, và chặn xóa `MaterialCategory` còn học liệu đang dùng. Lịch sử phát sinh thật (điểm danh, điểm, học phí, báo cáo) vẫn giữ.
