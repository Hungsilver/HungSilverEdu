...
- **Deploy có HTTPS sẵn (`docker-compose.https.yml` + `Caddyfile`):** build tại VPS + service **Caddy** (cổng 80/443) tự xin & gia hạn Let's Encrypt cho `h-edutech.io.vn`, ép HTTP→HTTPS, `reverse_proxy client:80`; client **không** publish cổng ra host. Chứng chỉ giữ ở volume `caddy_data`. Email ACME qua env `ACME_EMAIL`. Chạy: `docker compose -f docker-compose.https.yml up -d --build`. API không cần `ForwardedHeaders` vì cookie `Secure` set theo môi trường (Production) chứ không theo `Request.IsHttps`.
...
- **2026-06-19** — **Hạ tầng deploy HTTPS cho VPS** (xem §12): thêm `docker-compose.https.yml` (build tại VPS, full stack postgres+api+client+**Caddy**) + `Caddyfile` — Caddy tự cấp/gia hạn Let's Encrypt cho `h-edutech.io.vn`, ép HTTP→HTTPS, proxy về `client:80`; client bỏ publish cổng host; cert giữ ở volume `caddy_data`; thêm env `Cors__Origins__0=https://h-edutech.io.vn`. Không đổi code BE/FE. — `docker-compose.https.yml`, `Caddyfile`, `ARCHITECTURE.md`.
